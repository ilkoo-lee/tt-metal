...
ttnn::Tensor unified_routed_expert_moe(
    ...
    // into the existing dispatched buffer (instead of a freshly-zeroed output),
    // there is no per-call DRAM allocation and no up-front fill. Rows the FFN
    // writer does not touch (tile-aligned slack within a region, regions of
    // zero-count experts, and the tail of the buffer) retain their original
    // dispatched-buffer contents.
