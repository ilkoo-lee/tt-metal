...
UnifiedRoutedExpertFfnProgramFactory::cached_program_t UnifiedRoutedExpertFfnProgramFactory::create(
    ...
    for (uint32_t idx = 0; idx < cores.size(); ++idx) {
        ...
        const auto first_recv_noc = device->worker_core_from_logical_core(CoreCoord{origin_x + gx, origin_y + 1});
        ...
        //   5: leader_noc_x
        //   6: leader_noc_y
        //   7: leader_sync_sem_id
        //   8: num_non_leader_cores
        //   Global-sem trailing args (only consumed on the leader path):
        //   9: has_global_sem
        //  10..13: global_sem mcast bbox (start_x, start_y, end_x, end_y)
        //  14: global_sem_num_dests
        //  15: global_sem_l1_addr
        std::vector<uint32_t> writer_args = {
            out_buffer->address(),
            my_mt,
            my_nt_d,
            start_buffer->address(),
            (core == leader_logical) ? 1u : 0u,
            leader_noc_x,
            leader_noc_y,
            leader_sync_sem_id,
            num_non_leader_cores,
            has_global_sem,
            global_sem_mcast_start_x,
            global_sem_mcast_start_y,
            global_sem_mcast_end_x,
            global_sem_mcast_end_y,
            global_sem_num_dests,
            global_sem_l1_addr,
        };
