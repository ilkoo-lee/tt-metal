...
8. **B8 — CI perf gate for the DSA path** (promote a trimmed `test_sparse_mla_perf` off `@skipif(CI)` with a
   ...
   *(The perf tracy-subprocess deadlock is RESOLVED — see the 2026-06-24 progress entry; the harness now
   mirrors `tests/nightly/blackhole/sdpa`: impl `skipif(CI)`, gate patches `CI=false` for the subprocess.)*
9b. **B-topology — validate the sparse path on `ring`/`fabric2d` on a real Galaxy.** On the 8-chip box only
   `line` trains fabric; ring/fabric2d (and the `2x2`/`4x2` sub-meshes on Galaxy) are unverified for the
   sparse path. CI DSA jobs currently pin `line` pending this.
...
### 2026-06-24 (cont.) — MLA dense/sparse routing is config-determined, not runtime-seq-determined
A runtime argument must not change the model's op graph: traces record one forward's op order and replay
it with new data, so the dense-vs-sparse choice cannot depend on sequence length. Removed the
`end_pos > index_topk` branch in `mla.py::forward` (+ the `elif … write_k` warm-up). Now: `_has_indexer`
(config) ⇒ ALWAYS sparse (`_indexer.forward`, which also writes the K-cache), regardless of seq_len; else
dense. At seq_len <= index_topk the indexer top-k selects all causal keys, so sparse is numerically equal
to dense — same output, fixed op graph. Dense v3.1 (no indexer) is untouched (`else` → dense_single/chunked).
Device-verified (line-4x2, both variants, all seqs — seq256/2k now take the sparse path): DS seq256 0.9985 /
seq2k 0.997 / seq4k 0.997, GLM seq256 0.9992 / seq2k 0.9984 / seq4k 0.9975, KVPE ~0.9999, chunked ~0.9966.

### 2026-06-24 (cont.) — perf tracy deadlock: confirmed real, root-caused, fixed (reference-aligned)
The earlier "perf deadlock" was challenged as possibly a leftover-process artifact. Reproduced on a
freshly `tt-smi -r` reset board with all python killed: **still deadlocks** — hard evidence: the
`CHIP_IN_USE_2_PCIe` lock holder is the **parent `pytest -m perf` process** (PID confirmed), which ran
`test_mla_chunked_perf_impl` directly via the persistent `mesh_device` fixture and holds the lock at
process scope, so the gate's tracy subprocess can't acquire the board.
- **Root cause vs the reference** (`tests/nightly/blackhole/sdpa/test_ring_joint_sdpa.py`): that suite never
  runs the impl in the parent — the impl is `skipif(CI=="true")`, the gate opens **no device** and drives
  the impl by **node-id** via `run_device_profiler` wrapped in `mock.patch.dict(os.environ, {"CI":"false"})`,
  so only the subprocess (CI=false) opens the board. Our perf had no such gating (impl ran in the parent).
- **Fix (mirror the reference):** `test_mla_chunked_perf_impl` is now `skipif(CI=="true")`; the gate
  `test_mla_chunked_perf` patches `CI=false` around `run_device_profiler` and targets the impl by node-id.
  Dropped the interim `DS_PERF_RUN` toggle I had invented (reinvented exactly this).
- **Verified** on the reset board: the documented invocation `pytest -m perf …::test_mla_chunked_perf`
  passes in ~44 s, no lock-wait — LoudBox proxy 13.5 ms critical-path device-kernel / 163 op calls.
- **Remaining footgun:** a bare `pytest -m perf <file>` still collects the impl into the parent (the
  reference has no `-m perf` marker so it can't); the contract is "run the gate by node-id" (docstring).

### 2026-06-24 (cont.) — on-device verification of the renamed sparse suite + per-test timing
Ran the migrated tests (`test_sparse_mla.py`, `test_sparse_mla_vs_trace.py`, `test_sparse_mla_perf.py`,
post user-rename), shortest-first, on the 8-chip box. Per-test timings recorded in
`test_coverage_overview.md` (budget section).
- **Skip-by-default verified:** `-m`-less perf+trace = 106 skipped in **0.11 s** (no device), correct
  "require explicit marker" reasons. Both become selectable with `-m {trace,perf}`.
- **`test_sparse_mla` passes for both variants on the `line` fabric** (accuracy seq256/2k/4k, determinism,
  chunked). PCCs: DS/GLM output 0.997–0.999, KVPE ~0.9999, determinism pcc=1.0. Times: accuracy 7–12 s,
  determinism ~14–17 s, chunked ~10–12 s.
- **`-m trace` (explicit): 12 passed** — real official-trace parity (DS L0): logits 0.959, topk 0.985,
  output 0.9997, KV latent 0.998, k_pe frame-invariant L2 0.0089. (Trace bundles resolve in-tree from
  `bit_sculpt/results/`.)
- **`-m perf` (explicit): passes** when the tracy wrapper is run ALONE (`-k "not impl"`, ~80 s). Running the
  full `-m perf` (impl + wrapper in one session) deadlocks: the direct impl holds the device and the
  wrapper's tracy subprocess can't acquire it → timeout. The impl alone profiles fine (~10 s).
- **Environmental failures (NOT test logic):** `ring`/`fabric2d` fabric + `2x2` sub-mesh don't train on a
  partial-Galaxy 8-chip box (`Could not find any forwarding direction` / `Fabric Router Sync: Timeout`);
  `8x4` is Galaxy-only. See the topology note in the coverage doc.
- **Fixes:** added `(4,2)` to `SPARSE_MESH_PARAMS` (LoudBox full-box tp=2) so GLM is verifiable on a
  LoudBox — GLM passed on `line-4x2` (the original mesh set left GLM only on the broken `2x2`). Repointed
  the stale CI jobs (`test_mla.py`→`test_sparse_mla.py`, mesh ids `sp8xtp4`→`8x4`/`4x2`/`2x4`, pinned
  `line`); each selection collects exactly the 4 intended tests.
