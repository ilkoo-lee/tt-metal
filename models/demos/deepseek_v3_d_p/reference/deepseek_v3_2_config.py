...
def deepseek_v32_hf_config(max_seq: int = 16384):
    ...
    relying on host weights or a built cache. The CPU truth uses the default `ModelArgs` (its defaults
    already match these V3.2 dims), so no separate `cpu_model_args` is needed.
    """
