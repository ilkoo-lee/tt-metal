...
Dense MLA cache coverage lives in test_mla_cache.py. This sibling verifies the sparse-specific
...
mesh so the sparse epilogue / GLM (tp_cap=2) fit. Validity gating (so collected==run) is the same as
...
import ttnn
from models.demos.deepseek_v3_d_p.tests.dsa_reference import assert_config_matches, build_cpu_reference
...
def test_sparse_mla_cache_only_stays_sparse(mesh_device, device_params, variant, config_only):
    ...
    config = config_only
    args, _, weights, _ = build_cpu_reference(variant, SEQ_LEN)
