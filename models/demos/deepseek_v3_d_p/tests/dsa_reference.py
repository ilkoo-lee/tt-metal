...
def random_mla_weights(config, seed: int = 42) -> dict:
    """Config-driven random MLA + indexer weights (the same 12 keys as `WEIGHT_NAME_MAP`).

    Built straight from the HF `config` (not via MLACPU), so callers can override head counts on the
    config first — e.g. the perf harness forcing the Galaxy 128/64 head shapes. For PCC tests use
    `build_cpu_reference` instead (its weights come from the MLACPU instance that is also the truth).
    """
    ...
    idx_heads = getattr(config, "index_n_heads", 64)
    idx_dim = getattr(config, "index_head_dim", 128)

    def randn(shape):
        return (torch.randn(shape, generator=g, dtype=torch.bfloat16) * std).to(torch.bfloat16)

    return {
        "q_a_proj.weight": randn((config.q_lora_rank, h)),
        "q_a_layernorm.weight": torch.ones(config.q_lora_rank, dtype=torch.bfloat16),
        "q_b_proj.weight": randn((n_heads * qk_head_dim, config.q_lora_rank)),
        "kv_a_proj_with_mqa.weight": randn((config.kv_lora_rank + config.qk_rope_head_dim, h)),
        "kv_a_layernorm.weight": torch.ones(config.kv_lora_rank, dtype=torch.bfloat16),
        "kv_b_proj.weight": randn((n_heads * (config.qk_nope_head_dim + config.v_head_dim), config.kv_lora_rank)),
        "o_proj.weight": randn((h, n_heads * config.v_head_dim)),
        "indexer.wq_b.weight": randn((idx_heads * idx_dim, config.q_lora_rank)),
        "indexer.wk.weight": randn((idx_dim, h)),
        "indexer.k_norm.weight": torch.ones(idx_dim, dtype=torch.bfloat16),
        "indexer.k_norm_bias.weight": torch.zeros(idx_dim, dtype=torch.bfloat16),
        "indexer.weights_proj.weight": randn((idx_heads, h)),
    }
