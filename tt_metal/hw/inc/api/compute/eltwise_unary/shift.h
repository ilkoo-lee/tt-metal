 ...
 *
 * Out-of-range contract (matches torch): a shift amount outside [0, 31] produces 0 for every element;
 * callers do not need to clamp the shift amount themselves.
 *
 * Return value: None
 *
 ...
 * engine.
 *
 * Out-of-range contract (matches torch): a shift amount outside [0, 31] produces 0 for non-negative inputs and
 * -1 (all 1s) for negative inputs; callers do not need to clamp the shift amount themselves. Note this is an
 * arithmetic shift, so it is NOT a substitute for a logical right shift on negative/high-bit-set values.
 *
 * Return value: None
 *
 * | Argument        | Description                                                                | Type     | Valid Range                                           | Required |
